...
import { statusClasses } from "../../components/common/statusStyles";
...
export function DiagnosticCenter({
  ...
  const diagnosticItems: SortableGridItem[] = items.slice(0, 3).map((item) => ({
    id: item.id,
    node: (
      <article className={`h-full rounded-lg border p-3 ${statusClasses(item.level)}`}>
  ...
  return (
    <section className="rounded-lg border border-slate-200 bg-white p-4 shadow-sm">
      ...
      <SortableGrid
        ariaLabel="Diagnostic cards"
        className="grid gap-3"
        items={diagnosticItems}
        maxColSpan={2}
        storageKey="acc.diagnostics.issueCards"
      />
    </section>
  );
}
