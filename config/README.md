...
- `skill-tree-workshop/`：技能树工坊的只读展示配置。`manifest.json` 只登记 Agent，`agents/*.skill-tree.json` 为每个 Agent 的独立技能树图谱。

学习建议：

- 想改模型默认值、路径、端口、Agent 本机配置，通常从这里开始。
- 真正读取配置的代码入口在 `src/feishu_stack/core/settings.py`。
- 技能树展示配置的读取入口在 `src/feishu_stack/modules/agent_array/skill_tree/tree_config.py`。
