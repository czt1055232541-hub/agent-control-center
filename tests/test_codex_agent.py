...
class CodexAgentStartTests(unittest.TestCase):
    def test_already_running(self):
        cfg = _fake_config()
        with (
            patch('feishu_stack.codex_agent.read_pid', return_value=123),
            patch('feishu_stack.codex_agent.process_info', return_value=(True, 'node.exe')),
        ):
            from feishu_stack.codex_agent import start
            result = start(cfg)
    ...
    def test_build_fails(self):
        cfg = _fake_config()
        with (
            patch('feishu_stack.codex_agent.read_pid', return_value=None),
            patch('feishu_stack.codex_agent.process_info', return_value=(False, None)),
            patch('feishu_stack.codex_agent._build_if_needed', return_value=(False, 'build error')),
        ):
            from feishu_stack.codex_agent import start
            result = start(cfg)
    ...
    def test_start_success(self):
        ...
        with (
            patch('feishu_stack.codex_agent.read_pid', return_value=None),
            patch('feishu_stack.codex_agent.process_info', side_effect=[(False, None), (True, 'node.exe')]),
            patch('feishu_stack.codex_agent._build_if_needed', return_value=(True, 'built')),
            patch('feishu_stack.codex_agent.start_process') as mock_sp,
            patch('feishu_stack.codex_agent.write_pid'),
            patch('feishu_stack.codex_agent.time.sleep'),
        ):
    ...
    def test_start_exits_during_startup(self):
        ...
        with (
            patch('feishu_stack.codex_agent.read_pid', return_value=None),
            patch('feishu_stack.codex_agent.process_info', side_effect=[(False, None), (False, None)]),
            patch('feishu_stack.codex_agent._build_if_needed', return_value=(True, 'built')),
            patch('feishu_stack.codex_agent.start_process') as mock_sp,
            patch('feishu_stack.codex_agent.write_pid'),
            patch('feishu_stack.codex_agent.time.sleep'),
...
class CodexAgentStopTests(unittest.TestCase):
    def test_stop(self):
        cfg = _fake_config()
        with patch('feishu_stack.codex_agent.stop_component') as mock_stop:
            mock_stop.return_value = OperationResult(True, 'codex-agent', 'stop', 'stopped')
            from feishu_stack.codex_agent import stop
            result = stop(cfg)
            self.assertTrue(result.ok)
...
class CodexAgentRestartTests(unittest.TestCase):
    def test_restart(self):
        cfg = _fake_config()
        with (
            patch('feishu_stack.codex_agent.stop') as mock_stop,
            patch('feishu_stack.codex_agent.start') as mock_start,
        ):
            mock_stop.return_value = OperationResult(True, 'x', 'stop', 'ok')
            mock_start.return_value = OperationResult(True, 'x', 'start', 'ok')
            from feishu_stack.codex_agent import restart
            result = restart(cfg)
            self.assertTrue(result.ok)
