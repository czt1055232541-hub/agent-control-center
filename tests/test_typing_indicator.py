...
class TypingIndicatorStartTests(unittest.TestCase):
    def test_already_running(self):
        cfg = _fake_config()
        with (
            patch('feishu_stack.typing_indicator.read_pid', return_value=123),
            patch('psutil.Process') as mock_psutil,
        ):
            mock_proc = MagicMock()
            mock_proc.is_running.return_value = True
            mock_psutil.return_value = mock_proc
            from feishu_stack.typing_indicator import start
            result = start(cfg)
    ...
    def test_start_success(self):
        cfg = _fake_config()
        with (
            patch('feishu_stack.typing_indicator.read_pid', return_value=None),
            patch('feishu_stack.typing_indicator.start_process') as mock_sp,
            patch('feishu_stack.typing_indicator.write_pid'),
            patch('feishu_stack.typing_indicator.time.sleep'),
            patch('psutil.Process') as mock_psutil,
        ):
...
class TypingIndicatorStopTests(unittest.TestCase):
    def test_stop(self):
        cfg = _fake_config()
        with patch('feishu_stack.typing_indicator.stop_component') as mock_stop:
            mock_stop.return_value = OperationResult(True, 'typing-indicator', 'stop', 'stopped')
            from feishu_stack.typing_indicator import stop
            result = stop(cfg)
            self.assertTrue(result.ok)
...
class TypingIndicatorRestartTests(unittest.TestCase):
    def test_restart(self):
        cfg = _fake_config()
        with (
            patch('feishu_stack.typing_indicator.stop') as mock_stop,
            patch('feishu_stack.typing_indicator.start') as mock_start,
        ):
            mock_stop.return_value = OperationResult(True, 'x', 'stop', 'ok')
            mock_start.return_value = OperationResult(True, 'x', 'start', 'ok')
            from feishu_stack.typing_indicator import restart
            result = restart(cfg)
            self.assertTrue(result.ok)
