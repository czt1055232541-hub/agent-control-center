...
class CodexDesktopExecutablePathTests(unittest.TestCase):
    def test_from_status(self):
        ...
        with patch('feishu_stack.codex_desktop.codex_desktop_status', return_value=status):
            from feishu_stack.codex_desktop import _executable_path
    ...
    def test_fallback_install_location(self):
        ...
        with (
            patch('feishu_stack.codex_desktop.codex_desktop_status', return_value=status),
            patch('feishu_stack.codex_desktop._install_location', return_value=None),
        ):
            from feishu_stack.codex_desktop import _executable_path
            exe = _executable_path()
...
class CodexDesktopStartTests(unittest.TestCase):
    def test_already_running(self):
        ...
        with patch('feishu_stack.codex_desktop.codex_desktop_status', return_value=status):
            from feishu_stack.codex_desktop import start
    ...
    def test_no_executable(self):
        ...
        with (
            patch('feishu_stack.codex_desktop.codex_desktop_status', return_value=status),
            patch('feishu_stack.codex_desktop._executable_path', return_value=None),
        ):
            from feishu_stack.codex_desktop import start
            result = start(_fake_config())
    ...
    def test_start_becomes_ready(self):
        ...
        with (
            patch('feishu_stack.codex_desktop.codex_desktop_status', side_effect=[not_running, running]),
            patch('feishu_stack.codex_desktop._executable_path', return_value='C:\\Codex\\Codex.exe'),
            patch('subprocess.Popen') as mock_popen,
            patch('feishu_stack.codex_desktop.time.sleep'),
        ):
            from feishu_stack.codex_desktop import start
            result = start(_fake_config())
            self.assertTrue(result.ok)
...
class CodexDesktopStopTests(unittest.TestCase):
    def test_stop_success(self):
        ...
        with (
            ...
            patch('feishu_stack.codex_desktop.is_codex_desktop_running', return_value=False),
        ):
            mock_run.return_value = subprocess.CompletedProcess([], 0, 'SUCCESS', '')
            from feishu_stack.codex_desktop import stop
    ...
    def test_stop_failure(self):
        ...
        with (
            patch('subprocess.run') as mock_run,
            patch('feishu_stack.codex_desktop.is_codex_desktop_running', return_value=True),
        ):
...
class CodexDesktopRestartTests(unittest.TestCase):
    def test_restart(self):
        cfg = _fake_config()
        with (
            patch('feishu_stack.codex_desktop.stop') as mock_stop,
            patch('feishu_stack.codex_desktop.start') as mock_start,
        ):
            mock_stop.return_value = OperationResult(True, 'x', 'stop', 'ok')
            mock_start.return_value = OperationResult(True, 'x', 'start', 'ok')
            from feishu_stack.codex_desktop import restart
            result = restart(cfg)
            self.assertTrue(result.ok)
...
class CodexDesktopLogTests(unittest.TestCase):
    def test_log_writes_and_tails(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with patch('feishu_stack.codex_desktop.codex_desktop_status', return_value=status):
                from feishu_stack.codex_desktop import log
                result = log(cfg)
