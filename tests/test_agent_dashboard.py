...
from feishu_stack.agent_config_editor import backup_config, get_editable_config, rollback_latest, update_editable_config
from feishu_stack.agent_dashboard import dashboard_summary, explained_diagnostics, list_agents, list_infrastructure
from feishu_stack.agent_registry import load_registry
...
def _write_openclaw(root: Path) -> None:
    ...
    for agent_id, _, _, workspace in agent_rows:
        ...
        (sessions_dir / "sessions.json").write_text(
            json.dumps(
                {
                    f"agent:{agent_id}:feishu:group:oc_secret": {
    ...
    config = {
        ...
        "bindings": [
            {"agentId": "main", "match": {"channel": "feishu", "accountId": "ou_secret"}},
            {"agentId": "orchestrator", "match": {"channel": "feishu", "accountId": "ou_secret"}},
            {"agentId": "coordinator", "match": {"channel": "feishu", "accountId": "ou_secret"}},
            {"agentId": "archivist", "match": {"channel": "feishu", "accountId": "ou_secret"}},
        ],
    }
...
def _write_codex_agent(root: Path) -> None:
    ...
    peers = [
        {"name": "项目调度官", "openId": "ou_secret_1", "description": "coordinator"},
        {"name": "代码执行官", "openId": "ou_secret_2", "description": "developer"},
        {"name": "运维验证官", "openId": "ou_secret_3", "description": "ops-validation"},
        {"name": "质量审计官", "openId": "ou_secret_4", "description": "quality-auditor"},
        {"name": "项目档案官", "openId": "ou_secret_5", "description": "archivist"},
    ]
    (root / ".env").write_text(
        "\n".join(
            [
                ...
                "CODEX_AGENT_ARGS=exec --skip-git-repo-check",
                "LARK_BOT_OPEN_ID=ou_code_secret",
...
def test_agent_api_shape_does_not_leak_sensitive_values(tmp_path: Path) -> None:
    ...
    assert len(agents) == 5
    assert "ou_secret" not in payload
...
def test_dashboard_summary_counts_real_agents_only(tmp_path: Path, monkeypatch) -> None:
    monkeypatch.setattr("feishu_stack.agent_dashboard.recent_operations", lambda: [])
...
def test_codex_editable_config_updates_with_backup_and_rollback(tmp_path: Path) -> None:
    ...
    assert "ou_secret" not in json.dumps(after, ensure_ascii=False)
...
def test_editable_config_rejects_unsupported_or_sensitive_fields(tmp_path: Path) -> None:
    ...
    try:
        update_editable_config("codex-code-agent", {"LARK_BOT_OPEN_ID": "ou_secret"}, cfg)
