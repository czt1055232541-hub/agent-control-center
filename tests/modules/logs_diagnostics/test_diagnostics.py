...
class DiagnosticsMoonbridgeModelsTests(unittest.TestCase):
    def test_success(self):
        ...
        with patch('feishu_stack.modules.logs_diagnostics.diagnostics.urllib.request.urlopen', return_value=mock_resp):
            from feishu_stack.modules.logs_diagnostics.diagnostics import moonbridge_models
            cfg = MagicMock()
            cfg.raw = {}
    ...
    def test_http_error(self):
        import urllib.error
        with patch('feishu_stack.modules.logs_diagnostics.diagnostics.urllib.request.urlopen', side_effect=urllib.error.HTTPError('url', 500, 'err', {}, None)):
            from feishu_stack.modules.logs_diagnostics.diagnostics import moonbridge_models
            cfg = MagicMock()
            cfg.raw = {}
            cfg.moonbridge_port = 38440
    ...
    def test_connection_error(self):
        with patch('feishu_stack.modules.logs_diagnostics.diagnostics.urllib.request.urlopen', side_effect=ConnectionError('refused')):
            from feishu_stack.modules.logs_diagnostics.diagnostics import moonbridge_models
            cfg = MagicMock()
            cfg.raw = {}
            cfg.moonbridge_port = 38440
...
class DiagnosticsSummaryTests(unittest.TestCase):
    def test_diagnostics_aggregates(self):
        with (
            ...
            patch('feishu_stack.modules.logs_diagnostics.diagnostics.moonbridge_models') as mock_mm,
