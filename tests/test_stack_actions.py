...
class StackActionsCombineTests(unittest.TestCase):
    def test_all_ok(self):
        from feishu_stack.stack_actions import _combine
        import time
        t0 = time.monotonic()
        results = [
...
class StackActionsStartNativeTests(unittest.TestCase):
    def test_start_native(self):
        with (
            patch('feishu_stack.stack_actions.codex_provider.switch_provider') as mock_sp,
            patch('feishu_stack.stack_actions.openclaw.start') as mock_oc,
            patch('feishu_stack.stack_actions.codex_agent.start') as mock_ca,
            patch('feishu_stack.stack_actions.typing_indicator.start') as mock_ti,
        ):
            ...
            from feishu_stack.stack_actions import start_native
            cfg = MagicMock()
...
class StackActionsStartMoonbridgeTests(unittest.TestCase):
    def test_start_moonbridge(self):
        with (
            patch('feishu_stack.stack_actions.moonbridge.start') as mock_mb,
            patch('feishu_stack.stack_actions.codex_provider.switch_provider') as mock_sp,
            patch('feishu_stack.stack_actions.openclaw.start') as mock_oc,
            patch('feishu_stack.stack_actions.codex_agent.start') as mock_ca,
            patch('feishu_stack.stack_actions.typing_indicator.start') as mock_ti,
            ...
            from feishu_stack.stack_actions import start_moonbridge
            cfg = MagicMock()
...
class StackActionsStopTests(unittest.TestCase):
    def test_stop(self):
        with (
            patch('feishu_stack.stack_actions.typing_indicator.stop') as mock_ti,
            patch('feishu_stack.stack_actions.codex_agent.stop') as mock_ca,
            patch('feishu_stack.stack_actions.moonbridge.stop') as mock_mb,
            patch('feishu_stack.stack_actions.openclaw.stop') as mock_oc,
        ):
            ...
            from feishu_stack.stack_actions import stop
            cfg = MagicMock()
            result = stop(cfg)
            self.assertTrue(result.ok)
