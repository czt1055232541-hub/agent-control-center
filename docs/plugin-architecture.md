...
当前任务战场、本地工具、配置中心、路由规则、飞书连接、Agent 阵列和备份迁移使用此方式。Dashboard、Provider 和日志诊断的组合视图尚在主应用中，待迁移。Agent 页面拥有自己的选择状态和数据读取；备份迁移页面提供现有迁移与清理接口的入口，清理需显式勾选确认。外部独立发行包仍可使用下面的同源页面链接方式，无需参与 ACC 前端编译。
