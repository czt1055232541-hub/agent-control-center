...
共享契约版本 1 包括 Codex 路径/模型、MoonBridge 端口、OpenClaw 端口、Agent 目录、外置 `projectsRoot` 和 lark-cli 路径。根配置是唯一真值来源；`agent-runtime` 不保留第二份活动配置。

可用环境覆盖：

- `STACK_SETTINGS_PATH`：ACC 配置文件
- `FEISHU_AGENT_SETTINGS_PATH`：仅用于回滚审计时显式对比旧仓配置
