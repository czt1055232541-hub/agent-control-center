...
function AccOverviewCard({ block, inspect }) {
  ...
  const accent = model.online ? 'var(--dsw-alias-state-success-primary, #15803d)' : 'var(--dsw-alias-state-error-primary, #b91c1c)'
  const open = () => {
  ...
  return React.createElement('article', {
    ...
    style: {
      border: '0.5px solid var(--dsw-alias-border-l2, #d8dee9)', borderRadius: 12,
      background: 'var(--dsw-alias-bg-layer-1, #fff)', overflow: 'hidden', color: 'var(--dsw-alias-label-primary, #172033)',
    },
  },
  React.createElement('button', {
    type: 'button', onClick: open, disabled: !model.online || !model.url,
    style: { width: '100%', border: 0, background: 'transparent', color: 'inherit', fontFamily: 'inherit', padding: 14, textAlign: 'left', cursor: model.online ? 'pointer' : 'default' },
  },
  ...
  model.plugins.length ? React.createElement('div', { style: { display: 'flex', gap: 6, flexWrap: 'wrap', marginTop: 10 } },
    ...model.plugins.slice(0, 6).map(name => React.createElement('span', { key: name, style: { padding: '3px 7px', borderRadius: 999, background: 'var(--dsw-alias-interactive-bg-hover, #f1f5f9)', fontSize: 11 } }, name)),
  ) : null),
  inspect ? React.createElement('button', { type: 'button', onClick: inspect, style: { border: 0, borderTop: '0.5px solid var(--dsw-alias-border-l2, #d8dee9)', width: '100%', padding: '8px 14px', background: 'transparent', color: 'inherit', fontFamily: 'inherit', textAlign: 'left', fontSize: 12, opacity: .65, cursor: 'pointer' } }, '查看调用详情') : null)
}
