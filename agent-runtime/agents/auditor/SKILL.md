...
**来源：** 2026-08-10 复盘（S13）

**规则：** 审计归档报告时，必须检查是否存在未提交改动（WIP）的处置决定记录。若代码执行官或项目档案官在归档时报告了活跃目录中的 WIP 文件，审计结论必须确认：
- 每个 WIP 文件/目录是否标注了处置决定：保留 / 不提交 / 不删除
- 决定是否附带了路径和原因
- 决定是否合理（如"保留"是否说明了保留期限或用途）

**审计清单新增项：**
```
- WIP 处置：是否记录了未提交改动的处置决定（路径 + 决定 + 原因）
```

**判定：** 缺少 WIP 处置记录或记录不完整，审计结论至少为"有条件通过"，要求项目档案官补全后再归档。
