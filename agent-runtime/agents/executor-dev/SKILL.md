...
**来源：** 2026-08-10 复盘（S11）

**规则：** 发现 git 未跟踪或未注册 taskId 的功能模块时，必须按以下两步处理：

**第一步 — 补登记：**
- 向项目调度官报告发现：模块路径 + 文件清单 + 功能描述
- 由项目调度官补登记 taskId 并在 task_directory.json 中注册

**第二步 — 收尾执行：**
收到补登记后的收尾任务后，按以下子项逐项完成：
1. 杂物清理：删除临时文件、调试输出、注释掉的死代码
2. 测试验证：`python -m py_compile` 通过 + 主流程自测
3. 配置核对：检查硬编码路径、环境变量依赖、是否引用外部未登记资源
4. WIP 决策：未提交改动明确标注保留/不提交/不删除

### S12 — 清理区分对象

**来源：** 2026-08-10 复盘（S12）

**规则：** 执行清理操作前必须区分两类目标：
- **副本/临时目录：** 可安全删除，删除后附 `Test-Path` 证据
- **活跃运行时目录（agent.dir 指向）：** 不得删除，清理报告必须明确标注"活跃目录，保留"

**自检清单新增项：**
```
6. 清理安全：是否排除了 agent.dir 指向的活跃目录，删除报告是否附 Test-Path 证据
```
...
**规则：** 清理任务执行前必须读取 `stack.settings.local.json` 中的 `agent.dir` 配置，将对应目录加入排除列表。

### P9 — 归档后更新自测清单

**来源：** 2026-08-10 复盘（P9）

**规则：** 归档后更新场景的自测必须包含与归档报告的一致性检查：
- 端口号是否与归档记录一致
- 产物路径是否与归档记录一致
- 版本号/构建号是否递增
- 产物大小是否合理变化（附 sha256）
