...
**规则：** 报告"目录已删除"或"文件已移除"前，必须做到以下三点：
1. 区分根目录副本与活跃运行时目录（`agent.dir` 指向的目录）
2. 附命令行证据：`Test-Path <路径>` 或 `Get-ChildItem <路径>`
3. 明确列出检查了哪些路径、哪些依然存在、哪些已确认删除

**示例：**
```
验证范围：local_tools 清理
活跃目录（agent.dir=C:\agent-runtime\agents\local_tools）：Test-Path = True，保留
副本目录（C:\temp\local_tools_copy）：Test-Path = False，已删除
```

**判别：** 表述不实（声称删除但活跃副本仍存在）直接判定为返工项，质量审计官可要求重验。

### P8 — 清理脚本排除活跃目录

**来源：** 2026-08-10 复盘（P8）

**规则：** 执行任何清理验证前，必须先读取 `stack.settings.local.json` 中的 `agent.dir` 配置，将对应目录加入排除列表。清理操作只针对明确指定的目标路径，不得触碰 agent 活跃运行时目录。
